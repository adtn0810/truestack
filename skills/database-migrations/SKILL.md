...
`project-memory`. This is **ask-first, high-risk** work: route the plan through
`architecture-planning`'s approval gate **before** writing any migration, and remember the
...
`backend-development`; sequencing the migrate step in the deploy pipeline is `ci-and-delivery`.
...
40-minute production rewrite. Implement the scripts via `backend-development`; run
`quality-control` before "done". If the change altered a recorded command or decision, update
