...
If none exists, run `project-memory`. A deploy, image, or cert change is **high-risk and
irreversible-shaped** — route the plan through `architecture-planning`'s approval gate before
...
**Where this skill sits:** the app/server logic is `backend-development`; this skill packages and
runs that app on the box. The CI workflow that *triggers* this deploy is `ci-and-delivery`; the
migration DDL it runs is `database-migrations`; the app's instrumentation/SLO signal is
`observability` (this skill owns the container/compose healthcheck and the cutover readiness gate).
...
When the Dockerfile/compose/nginx/systemd config changes, hand off to **quality-control** before
