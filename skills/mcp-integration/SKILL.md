...
description: Act on external systems through connected MCP servers — query or write a
  ...
  discipline to tool calls as backend-development applies to code.
---
...
boundary rule `backend-development` and the security checklist enforce for any external input.
...
- **Bound it**: page/limit large reads (don't pull an unbounded result set into context); set timeouts; cap batch sizes — the same anti-overload discipline as `backend-development`.
...
When integration work changes code or config, hand off to **quality-control** — its security and intent
