...
already recorded. If none exists, run `project-memory`. Keep the **code↔memory tally** balanced:
...
decision — route a consequential one through **architecture-planning**'s approval gate. (The enforced
...
lifecycle*. **`ci-and-delivery`** SHA-pins its *own* GitHub Actions and runs the scanner *step* in
CI — but the policy that step enforces (pin mode, cooldown, the fail-on bar, the SBOM artifact)
is *this* skill's. **`quality-control`** runs the one-shot SAFETY pass on a given change (including
"this bump introduces a known CVE"); **application-security** owns the security DESIGN/discipline;
this skill owns the standing dependency posture between changes. **`mcp-integration`** owns
untrusted *tool-output*; the PreToolUse hook governs agent *actions* — both are different layers.
**`observability`** redacts PII in telemetry; **data-privacy** owns the privacy policy — neither is
license/SBOM. **`backend-development`** justifies *why* a dep is needed at code time; this skill
...
## Honest exit
SHA-pinning Actions (which `ci-and-delivery` already does) closes the CI-runner chain — it does
