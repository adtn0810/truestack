...
boundaries. If none exists, run `project-memory`.

For a real external action — DB write, vector search, payment, webhook, third-party API —
go through **`mcp-integration`** rather than hand-rolling the call: it carries the
...
(Same clarify-then-proceed loop as `architecture-planning`.)
...
criteria. After implementing, run `quality-control` (its intent check maps each criterion to evidence):
...
When implementation is complete, hand off: run **quality-control** before calling it done.
