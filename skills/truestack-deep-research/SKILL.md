...
description: Run a thorough multi-source research pass and produce a verified, cited answer
  ...
  "what's the best/latest", "is it still true that", or asks "how does X achieve / do this"
  about a third-party system (nothing shared to inspect), or needs a recommendation backed
  by sources. Fans out parallel searches, reads primary sources, cross-checks each
  load-bearing claim, and separates verified from uncertain. Pairs with
  truestack-agent-coordination for the fan-out.
---
...
in `truestack-project-memory`'s honesty reference (§8 of its `honesty` doc) — it escalates to
this full pass for high-stakes or wide-open questions.
