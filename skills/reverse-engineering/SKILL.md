...
description: Reverse-engineer a reference — a repo, code snippet, doc/spec, or a legacy system —
  ...
  through architecture-planning's approval gate and quality-control.
---
...
If none exists, run `project-memory`. **Authorized targets only**: the reference must be something
...
confirmation** (same bar as `root-cause-debugging`). Tag each finding **verified** (saw it happen)
...
**high-risk-shaped** — route the plan through **architecture-planning**'s approval gate **before any
code**, especially where it touches accuracy-critical paths, data, money, or schema. Hand
implementation to **backend-development** (or the right builder), gate "done" through
**quality-control**, and keep the **code↔memory tally** balanced — record the decision *and its
...
gated upgrade plan to **architecture-planning** → **backend-development** → **quality-control**; for
deep external/landscape research, pair with **deep-research**.
