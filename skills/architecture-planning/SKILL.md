...
this skill produces an architecture the user signs off on before `backend-development` runs. Aim
...
`project-memory` before planning anything substantial.

## Right-size the work (do this first)
Match the ceremony to the size of the change — heavy process on a small task is its own
failure mode:
- **Trivial** (rename, copy tweak, one-liner) **and unambiguous**: skip planning; go straight to `backend-development` + `quality-control`. If even a small ask is ambiguous, clarify first (see step 6).
...
implemented and checked on its own. This gives `backend-development` a way to validate as it goes
...
implementation. Hand a parallelizable breakdown to `agent-coordination`.
...
via **project-memory** so the code↔memory tally starts balanced. When the plan is approved, hand
off: proceed to **backend-development**.
