...
box. If none exists, run `project-memory`. Keep the **code↔memory tally** balanced: when you
...
client depends on a shape, you own it. Route the contract through `architecture-planning`'s
approval gate before it ships, and let `quality-control` gate it before "done". Don't guess on a
...
- `api-design` owns the **interface contract**; `backend-development` **implements** it (owner-scoped
  auth, validation, the idempotency table). Define the shape here, build it there.
- `architecture-planning` does **system** design (modules, data flow, services); this skill designs
...
- This skill owns the **authorization rules in the contract** (BOLA/BOPLA below) — the design
  discipline. `quality-control` runs the one-shot **SAFETY pass** that checks a given change against
  OWASP; it doesn't author the contract.
- `mcp-integration` owns **untrusted tool output** crossing into the app; this skill owns the
  request/response schema that validates **client** input at the edge.
- `observability` owns **PII redaction in telemetry**; keep PII out of URLs/query strings here so it
  never reaches a log in the first place, but the redaction policy is theirs.
- `ci-and-delivery` runs the spec lint + backward-compat diff **as a CI gate**; this skill defines
...
`backend-development` enforces these at runtime; the contract is where they're *specified*.
...
customer. `backend-development` builds the table; the contract names the header and the rules.
...
`ci-and-delivery` wires these as the actual PR checks; this skill supplies the definition they enforce.
...
memory in the same change; then hand off to **quality-control** before "done".
...
that it couldn't before — and hand off to **backend-development** to implement it.