...
`project-memory`. Keep the **code↔memory tally** balanced: a new data class, retention rule,
...
route the design through `architecture-planning`'s approval gate before building, and
...
every neighbor. `quality-control` runs the one-shot SAFETY pass on a change and
`application-security` owns the security *design*; this skill owns the data-handling *rules*
they enforce. `observability` redacts PII in telemetry but **defers the classification and
redaction policy to here** — it implements; this skill decides what is PII. `mcp-integration`
owns untrusted tool-output; the PreToolUse hook governs agent *actions* (a different layer).
`ci-and-delivery` runs the dependency scan in CI but `dependency-management` owns the dep
lifecycle; `backend-development` implements the deletion/consent APIs (and `database-migrations`
authors the purge/erasure DDL + bounded backfill) but `api-design` owns their contract;
`architecture-planning` does system design.
...
Define the policy `observability` enforces: **deny-by-default** — structured fields only,
...
bodies. This skill defines what's PII; `observability` does the scrubbing.
...
retention rule, or basis changes; then hand off to **quality-control** before "done".
