...
(`architecture-planning`, `backend-development`, `root-cause-debugging`, `quality-control`, `react-frontend`, `mcp-integration`) all read
this first.
...
└── lessons.md         # non-obvious FUNCTIONAL-bug gotchas (one line each), written by root-cause-debugging
```
...
contracts, migrations, lockfiles, or root config unless assigned. See the `agent-coordination` skill.
...
idempotency keys on retried writes and verify the real effect afterward. See `mcp-integration`.
...
flagged. (Full contract: `project-memory/references/honesty.md`.)
...
engine is `deep-research`; escalate to its full cited pass for high-stakes or wide-open questions.
...
- **The tally must balance to be "done".** A change that leaves memory contradicting the code is
  unfinished — `quality-control` checks this reconciliation and `orchestrate` won't call work done
  until it balances.
