...
5. After all merges, run `truestack-quality-control` on the integrated branch; clean up worktrees (`git worktree remove`).
...
|-----------|--------------------------------------------------|-----------|----------------------------------------|--------------|
| review    | security · performance · architecture · tests    | none (RO) | all parallel                           | synthesize by severity → truestack-quality-control |
| debug     | hypothesis×3 → fixer                              | fixer only| hypotheses parallel, fix sequential    | first confirmed cause wins; regression test |
| feature   | contracts → backend ∥ frontend → tests           | per writer| contracts first, then parallel, then tests | merge backend→frontend→tests; truestack-quality-control |
