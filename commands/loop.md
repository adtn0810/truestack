---
description: Iterate to the goal — keep building/fixing and re-running quality-control until the acceptance criteria pass (or report an honest completion score if blocked).
...
Use the **build loop** (backend-development) for a feature or the **verify-or-loop** (root-cause-debugging) for a
bug: implement/fix → run **quality-control** → if a criterion or the repro still fails, identify exactly
