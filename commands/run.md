...
(see quality-control `references/tooling.md`) — e.g. jest/vitest, `dotnet test`, or pytest; build and
lint as asked. Show each command → result in a small table. On failure, surface the actual
error and hand off to **root-cause-debugging**; never report a pass you didn't run.
